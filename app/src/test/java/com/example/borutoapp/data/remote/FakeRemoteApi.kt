...
class FakeRemoteApi : BorutoApi {
    ...
    private fun findHeroes(name: String): List<Hero> {
        val noHeroes = emptyList<Hero>()
        if (name.isEmpty() || name.isBlank())
            return noHeroes
        val heroesFound = heroes.filter { it.name.lowercase().contains(name.lowercase()) }
        return if (heroesFound.isNotEmpty()) heroesFound else noHeroes
    }
}